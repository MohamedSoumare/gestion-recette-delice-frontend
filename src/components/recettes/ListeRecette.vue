<template>
  ...
  <div class="d-flex">
    <input v-model="searchQuery" class="form-control form-control-sm me-2" :placeholder="$t('search_placeholder')" />
      ...
      <div class="col-md-4 mb-4" v-for="item in filteredRecettes" :key="item.id">
        <div class="card h-100">
          <div class="card-body">
            ...
            <p class="card-text"><strong>{{ $t("ingredients") }}:</strong> {{ item.ingredients }}</p>
            <p class="card-text"><strong>{{ $t("type") }}:</strong> {{ item.type }}</p>
            <p class="card-text"><strong>{{ $t("category") }}:</strong> {{ item.categorie }}</p>
          </div>
